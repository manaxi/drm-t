    ...
    createRouter: function createRouter() {
      ...
      router.get('/:videoName', function processGet(request, response) {
        ...
        if (!secretManagement.areSecretsAvailable()) {
          console.log(
            'ERROR: You must configure the secrets file to generate license tokens.'
          );
          response
            .status(NEED_TO_KNOW_SECRETS_STATUS_CODE)
            .send(
              'You must configure the secrets file to generate license tokens.'
            );
          return;
        }
        ...
        video.keys.forEach(function(key) {
          ...
          let encryptor = crypto.createCipheriv(
            'aes-256-cbc',
            communicationKeyAsBuffer,
            keyIdAsBuffer
          );
          ...
          encryptedKeyAsBuffer = Buffer.concat([
            encryptedKeyAsBuffer,
            encryptor.final()
          ]);
        ...
        console.log(
          'Creating license token with payload: ' + JSON.stringify(envelope)
        );
